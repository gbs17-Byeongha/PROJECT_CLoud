        ...
        for instance in reservation['Instances']:
            ...
            if is_postgresql_open:
                if public_ip and is_public_subnet:
                    status = "FAIL"
                    severity = "CRITICAL"
                    status_extended = f"Instance {instance_id} has PostgreSQL port 5432 open to the Internet and is in a public subnet with a public IP."
                elif public_ip:
                    status = "FAIL"
                    severity = "HIGH"
                    status_extended = f"Instance {instance_id} has PostgreSQL port 5432 open to the Internet and has a public IP but is in a private subnet."
                else:
                    status = "FAIL"
                    severity = "MEDIUM"
                    status_extended = f"Instance {instance_id} has PostgreSQL port 5432 open to the Internet but has no public IP."
            else:
                status = "PASS"
                severity = "INFO"
                status_extended = f"Instance {instance_id} does not have PostgreSQL port 5432 open to the Internet."
